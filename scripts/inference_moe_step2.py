...
parser.add_argument("--step1-image", type=str, help="Name of the image after", required=True)
...
first_turn_image = Image.open(args.step1_image).convert("RGB")
...
combined_image.paste(first_turn_image, (width, 0))
combined_image.paste(first_turn_image, (width * 2, 0))
...
result_image = result_image.crop((width*2,0,width*3,height))

os.makedirs(args.output_dir, exist_ok=True)

image_name = args.image.split("/")[-1]
result_image.save(os.path.join(args.output_dir, f"{image_name}_add.jpg"))
